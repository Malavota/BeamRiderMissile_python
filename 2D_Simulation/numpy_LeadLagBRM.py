...
def calculate_derivatives_with_lead_lag(r_t, r_m, v_m, beta_t, x_filter_state):
    ...
    # Implementação da rede Lead-Lag via Espaço de Estados simplificado:
    # G(s) = 10 * (s + 2) / (s/20 + 1) -> reformatado para integração
    # nc = 100 * error_linear - 90 * x_filter_state (exemplo de ganhos ajustados)
    
    # Para simplificar, vamos usar a forma diferencial da rede:
    # d(x_filter)/dt = (error_linear - x_filter) / tau
    tau_lag = 1.0 / 20.0
    tau_lead = 1.0 / 2.0
    gain = 10.0
    ...
    x_nc = gain * (x_filter_state + tau_lead * dx_filter)
...
with open('DATFIL.txt', 'w') as f: # TODO verificar se é necessario
    while v_c >= 0: # Substitui o IF(VC<0.) GOTO 999
        ...
        v_tp, v_mp, a_mp, beta_p, xnc_p, dxf_p = calculate_derivatives_with_lead_lag(r_t, r_m, v_m, beta_t, x_filter)
        ...
        vt_c, vm_c, am_c, beta_c, xnc_c, dxf_c = calculate_derivatives_with_lead_lag(r_t, r_m, v_m, beta_t, x_filter)
