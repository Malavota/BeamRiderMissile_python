...
def calculate_derivatives_3d(r_t, r_m, v_m, beta_t):
    ...
    x_nc_theta = XNP * mod_rm * (theta_t - theta_m)
    x_nc_phi = XNP * mod_rm * (phi_t - phi_m)
    
    # 4. Vetor de Aceleração 3D
    # Direção da Linha de Visada (LOS)
    los_vector = r_tm / mod_rtm
    ...
    return v_t, v_m, a_m, betatd, np.sqrt(x_nc_theta**2 + x_nc_phi**2)
...
while v_c >= 0:
    ...
    # Preditor
    vt_p, vm_p, am_p, btd_p, g_p = calculate_derivatives_3d(r_t, r_m, v_m, beta_t)
    r_t += H * vt_p
    r_m += H * vm_p
    v_m += H * am_p
    beta_t += H * btd_p
    ...
    # Corretor
    vt_c, vm_c, am_c, btd_c, g_c = calculate_derivatives_3d(r_t, r_m, v_m, beta_t)
